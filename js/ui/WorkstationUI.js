/* =========================================================
   ...
   how the main Inventory screen works, including drag-
   distribute (pick up a stack, then drag across several slots
   to spread it evenly). Furnace and Chest can follow the same
   pattern: give them their own <template> in index.html, a
   matching _build*Layout() here, and a dedicated Inventory
   field to read/write.
   ========================================================= */

import { matchRecipe, consumeOne } from '../engine/Crafting.js';
import { buildSlotEl, renderPlayerGrids, syncHeldCursorIcon, createDragController, placeOrMerge } from './InventorySlots.js';
...
export class WorkstationUI {
  constructor(inventory, { onClose } = {}) {
    ...
    this.dragController = createDragController(this.held, () => this._render());
  }
  ...
  close() {
    ...
    // Same as InventoryUI: return anything still sitting in the crafting
    // grid, not just the held item — otherwise ingredients left in the
    // 3x3 grid become invisible/inaccessible until reopened.
    if (this.inventory.craftingGrid3x3) {
      for (let i = 0; i < this.inventory.craftingGrid3x3.length; i++) {
        const stack = this.inventory.craftingGrid3x3[i];
        if (stack) {
          this.inventory.addItem(stack.item, stack.count);
          this.inventory.craftingGrid3x3[i] = null;
        }
      }
    }
  ...
  _render() {
    renderPlayerGrids(this.backpackEl, this.hotbarEl, this.inventory, this.held, () => this._render(), this.dragController);
  ...
  _renderCraftingGrid() {
    ...
    this.inventory.craftingGrid3x3.forEach((stack, i) => {
      const el = buildSlotEl(stack, () => {
        placeOrMerge(this.held, this.inventory.craftingGrid3x3, i);
        this._render();
      });
      this.dragController.wireSlot(el, this.inventory.craftingGrid3x3, i);
