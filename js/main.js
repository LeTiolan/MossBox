...
function createGame() {
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (action?.startsWith('hotbar')) {
      inventory.selectHotbarSlot(Number(action.replace('hotbar', '')) - 1);
    }
  ...
  return { renderer, world, chunkManager, controls, interaction, inventory, hud, inventoryUI, workstationUI };
}
...
function loop() {
  ...
  if (gameInstance) {
    const { controls, interaction, chunkManager, renderer } = gameInstance;
    controls.update(dt);
    interaction.update();
