...
// (Lake placement's own dedicated noise sample/threshold now lives
// directly in _biomeAndHeightAt() below, replacing this constant.)
...
export class World {
  ...
  _biomeAndHeightAt(wx, wz) {
    // Reduced from 5 octaves — this is both a smoothness fix (fewer
    // octaves means less high-frequency jitter, i.e. less "spiky/blocky"
    // terrain — Forest was the one biome still using full jaggedness)
    // and a real performance win, since this runs once per column
    // (65536 times per chunk) and each octave is its own noise sample.
    const h = this.heightNoise.fbm2D(wx * 0.01, wz * 0.01, { octaves: 4 });

    // Lake placement now uses its OWN dedicated, much-lower-frequency
    // noise sample instead of thresholding the regular terrain height.
    // This is what actually controls SPACING (not just rarity) between
    // water bodies — a low frequency means the noise's dips are simply
    // farther apart in world-space, unlike raising/lowering a threshold
    // on the regular terrain noise, which only changes how much of the
    // existing (closely-spaced) dips qualify. Tuned for roughly a
    // 150-300 block gap between lakes — adjust the 0.0016 multiplier or
    // the threshold below if that's not close enough once tested live.
    const lakeVal = this.heightNoise.fbm2D(wx * 0.0016, wz * 0.0016, { octaves: 2 });
    const biome = lakeVal < -0.55 ? BIOME.LAKE : this.biomeGenerator.getBiome(wx, wz);

    // Plains/Desert/Lake all re-sample height with fewer octaves and a
    // smaller amplitude than Forest's hills. Plains: flat, open ground.
    // Desert: same, PLUS a fixed +8 offset so it can never dip below sea
    // level at all. Lake: capping depth keeps lakebeds shallow instead
    // of letting some lakes plunge down into true cave territory. Forest
    // itself is also toned down from its original amplitude (24 -> 14)
    // for the same smoothness reason as the octave reduction above.
    let heightSample = h;
    let amplitude = 14;
    let offset = 0;
