  ...
  _generateChunk(cx, cz) {
    ...
    for (let lx = 0; lx < CHUNK_SIZE; lx++) {
      for (let lz = 0; lz < CHUNK_SIZE; lz++) {
        ...
        // Lake threshold widened from -0.25 to -0.12 — the narrower band
        // was letting occasional land columns (Plains/Forest, whose
        // height noise landed just barely above the old cutoff) poke up
        // above the water surface in the middle of what should read as
        // open lake, since Plains/Forest/Desert are generated per-column
        // independent of their neighbors. This isn't a full fix (true
        // shoreline smoothing would need neighbor-aware height blending,
        // a bigger change) but meaningfully shrinks how often it happens
        // by converting more of that borderline terrain into proper
        // (dry, then correctly flooded) lakebed instead.
        const biome = h < -0.12 ? BIOME.LAKE : this.biomeGenerator.getBiome(wx, wz);

        // Plains/Desert/Lake all re-sample height with fewer octaves and
        // a smaller amplitude than Forest's jagged hills. Plains/Desert:
        // flat, open ground. Lake: capping depth keeps lakebeds shallow
        // (roughly SEA_LEVEL-8 at the very deepest) instead of the old
        // shared Forest amplitude letting some lakes plunge down into
        // true cave territory (y<40) — which is what was reading as
        // "water pockets" appearing underground: it wasn't the cave
        // system leaking water, it was a lake's own water column simply
        // extending unusually deep and overlapping cave depth.
        let heightSample = h;
        let amplitude = 24;
        ...
        const isDryLand = surfaceY >= SEA_LEVEL; // guards decor/trees below
        ...
        // Only ever placed on dry land: this column's own water-fill
        // (inside buildColumn, for any biome whose surface dips below sea
        // level) runs before this and would otherwise get silently
        // overwritten by a cactus/grass/flower sitting underwater.
        if (isDryLand) {
        ...
        // trees spawn right next to each other. Also dry-land-only, same
        // reasoning as decor above.
        if (biome === BIOME.FOREST && isDryLand) {
